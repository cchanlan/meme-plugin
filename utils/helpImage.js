...
export async function renderHelp (info = {}) {
  ...
  const admin = block('🔧', '管理 · 群管/主人', [
    ...
    ['#meme开关', '看本群当前状态'],
    ['#meme更新', local ? '拉新表情并刷新索引（仅主人）' : '同步服务方的新表情（仅主人）'],
    ['#meme刷新', '只重建索引（仅主人），#meme重载 同效'],
    ['#meme部署状态', '查服务健康度（仅主人）'],
    ['#meme清缓存', '清空预览图缓存（仅主人）'],
    ['#meme清空统计', '排行榜清零重来（仅主人）'],
    ['#meme插件更新', '更新插件本体（仅主人），非表情'],
    ['#meme版本', '看版本并查有没有新的（仅主人）'],
    ...(local ? [
      ['#meme部署', '本机装一套服务（仅主人，venv / docker）'],
      ['#meme卸载', '卸掉本机这套（仅主人），会先列清单确认']
    ] : [])
  ], 'lilac')
  ...
  const play = fun
    ? block('🎪', '随机整活', [
        ['#抽个cp', '随机两个群友配个双人表情'],
        ['#抽个cp @张三', '指定一位，另一位随机'],
        ['#整活 @某人', '连出好几个表情，#表情轰炸 同效'],
        ['#套娃 摸头 踩', '头像一层层叠进表情里'],
        ['#随机套娃', '不点名，随机叠几层'],
        ['#全群摸头', '随机几个群友一起做，#全员 #全体 同效'],
        ['#今日表情', '今天固定的表情 + 一句运势'],
        ['#今日运势', '同上，#每日表情 也行'],
        ['#猜表情', '出题让群友猜名字，答对得分'],
        ['#猜表情排行', '本群榜；#猜表情总排行 看跨群']
      ], 'lilac')
    : ''
