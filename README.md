...
- **搜索出真图** —— `#表情包搜索 鸣潮` 出的是带真实预览图的网格，同时匹配关键词、分类和英文代码。服务端自带的列表图只画关键词和占位图标，所以这张图是插件自己拼的
...
- **参数全支持** —— 直接读服务端的 JSON Schema，带参表情全部可传参，`#表情名详情` 能查支持哪些参数
...
| 指令 | 说明 |
|---|---|
| `#表情包搜索 猫` | 出预览图网格，超过 8 个会附带可复制的名字列表 |
| `#表情包列表` / `#表情包列表 3` | 分页浏览，走服务端出图（字大清晰、无预览图） |
| `#表情包分类` | 列出全部分类及各自表情数 |
| `#表情包分类 鸣潮` | 出该分类的预览图网格 |
| `#随机表情包` | 随机来一个 |
| `#表情包帮助` | 完整说明 |
...
两个配置项，锅巴面板里用标签输入，也可以直接改 `config/config.yaml`：
...
**`searchMaxPreview` 别调大。** QQ 气泡里图片最宽只显示约 420px，每格实际宽度就是
`420 ÷ 列数`：24 个约每格 70px，80 个（12 列）只剩 35px、字号 2.9px，名字彻底读不了。
想一次看全应该去 Web 预览站，调大这个数只会更糊。
...
```
apps/       指令入口（生成 / 列表 / 搜索 / 更新 / 部署 / 帮助）
model/      配置、API 封装、表情索引、预览图
utils/      参数解析、文件、网格出图、meme_dirs 同步、黑名单
server/     Web 预览站
