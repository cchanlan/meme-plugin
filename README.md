...
- **站内在线生成** —— 不用发到群里也能做：Web 站详情页传图（或填 QQ 号取头像）、填文字、选参数，点一下出图直接保存
...
- **一条指令更新** —— `#meme更新` 自动完成 拉仓库 → 登记 `meme_dirs` → 重启服务 → 热加载索引，不用重启 Yunzai；**连的是外部服务时自动只刷索引**，不会白拉几个 G 的仓库
...
> 用外部服务的话，**表情资源仓库那套东西全都跟你无关**：`repos`、`reposDir`、`gitProxy`
> 不用配，`#meme部署` 不用发。`#meme更新` 会自动识别（`serviceMode: auto` 按服务地址判断，
> 回环地址才算本机），只帮你刷新索引把服务方的新表情同步过来，不会去拉几个 G 的仓库
> —— 本机没有 meme-generator 会扫它，拉下来纯占磁盘。
>
> 唯一 auto 判不出来的情况：服务跑在**本机 docker** 里（地址是 `127.0.0.1` 但仓库和
> `config.toml` 在容器内），这种要手动把 `serviceMode` 设成 `remote`。
...
装完想撤掉发 `#meme卸载` —— 这是插件里**唯一会删东西的指令**，所以先出预检清单
（「会删」「不会动」两栏都带体积），发 `#meme卸载确认` 才真动手：

| | 内容 |
|---|---|
| 会删 | pm2 进程（**只删可执行文件或工作目录落在 `data/meme-plugin` 里的那个**）并 `pm2 save`；`data/meme-plugin/venv`；插件生成的 `config.toml`（优先还原部署时留下的 `.bak.*` 备份，没备份才删，备份本身留着）；回滚 `deployed`/`memePm2Name`/`memeApiUrl`/`serviceMode` |
| 不删 | 表情仓库 —— 默认保留，要连仓库一起删得发 `#meme卸载全部确认`，且 `reposDir` 指到插件目录外时一个字节都不动；表情素材目录（**Windows 上它和 `config.toml` 同一个目录**，删了会连配置一起没）；别人的 pm2 进程和 `config.toml`（首部没有插件写的标记就不认） |

光比进程名是不够的：`deployPm2Name` 被误填成别人的进程名时，按名字删会把人家手工装的服务连
pm2 dump 一起抹掉，所以要额外做路径归属校验。Windows 上顺序是**先停进程再删目录** ——
venv 里的 `meme.exe` 刚被停掉时句柄常还没释放，直接删会 EBUSY，删除带 6 次重试。

卸载最后会再探一次 `memeApiUrl`：**连不上才清掉本地索引和出图缓存**。索引（`keyMap.json`/
`infos.json`）是第二层缓存，留着的话重启后列表、搜索、Web 站照旧显示满满 944 个表情，一发指令
才报连不上 —— 「显示不出来」比「显示得出来但用不了」好懂。反过来，服务还连得上（比如你自己
另有一套、或 `memeApiUrl` 指着别的机器）就一个字节都不清，表情功能不受影响。
...
| `#爬#33` | 带参数，`#` 后面是参数值 |
| `#一直#循环` | 参数是枚举时直接写选项名 |
| `#摸头详情` / `#摸头帮助` | 出图看这个表情支持什么参数、默认文字是什么，带真实预览。两种后缀等价 |
...
| `#meme更新` | 仅主人 | 拉取新表情：git pull → 登记 `meme_dirs` → 重启 meme 服务 → 热加载索引。**用外部服务时自动退化成只刷索引**，不会去拉仓库 |
...
| `#meme卸载` | 仅主人 | `#meme部署` 的反向操作，卸掉插件装的那套（**唯一会删东西的指令**，见下） |
...
- 点卡片复制指令，详情弹层显示参数说明
- **按最近更新倒序**，新拉的表情排最前面
- **在线生成**：详情弹层里直接传图/填文字/选参数，点一下出图，长按或点「保存到相册」存下来
...
详情弹层底部的「🎨 在线生成」，`enableWebMake` 可关。每个图片位二选一：选本地文件，
或填一个 QQ 号直接取头像；文字框预填了该表情的默认文本，参数按类型给复选框/下拉/数字框。

出图是动图时按钮会写「⬇ 保存 GIF」并附一句提醒：**别用浏览器右键的「复制图像」**。
那个动作是把图解码成位图放进剪贴板（系统剪贴板只认单帧位图），粘到 QQ 就只剩第一帧、
变成静态图 —— 任何网站的 GIF 都这样，不是这里生成错了。存成 .gif 文件再发，
或者手机上长按图片保存，动画才在。

**这块是唯一会消耗算力的入口**，而站点没有鉴权，所以后端上了三道闸：

| 闸 | 值 | 为什么 |
|---|---|---|
| 每 IP 每分钟次数 | `webMakeLimit`，默认 10 | 挡脚本连点 |
| 同时进行的生成数 | 固定 2 | meme 服务是单进程 Python，堆并发只会一起变慢 |
| 单次请求体 | `maxFileSize × 1.4 × 图片位数`，封顶 50MB | 图走 base64 会涨 1/3；9 图的表情不封顶能算到 126MB |

不想让别人用就 `enableWebMake: false`，关掉后详情弹层里不显示这一块，接口也直接返回 403。
...
| `serviceMode` | `auto` | 服务归属：`auto` 按地址判断 / `local` 本机（更新时拉仓库+重启进程）/ `remote` 外部（更新时只刷索引） |
...
| `enableWebMake` | `true` | Web 站内在线生成开关（无鉴权，按 IP 限频） |
| `webMakeLimit` | `10` | 每个 IP 每分钟最多生成几次 |
...
| `reposDir` | 空 | 表情仓库目录，机器上已有仓库时**必须**填（仅本机服务用得上） |
| `repos` | 5 项 | 订阅的表情仓库，含 `memeSubDir`（仅本机服务用得上） |
| `gitProxy` | 空 | GitHub 前缀代理，**建议留空直连**（仅本机服务用得上） |
...
用外部 meme 服务的话这条与你无关 —— 插件不会替你拉仓库，见上面「meme 服务」那节。
