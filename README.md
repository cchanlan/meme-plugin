...
- `#meme搜索 猫` 出九宫格真图；`#meme列表` 随机翻一页；`#meme新增` 看最近装上的
...
| `#套娃 摸头 踩 恐龙` | 头像套进第一个表情，成品再套下一个，一层层叠，一条消息发全过程。不写表情名就随机叠（`#随机套娃` 同效，会自动挑动图，别叠出一张不动的） |
| `#全群摸头` | 同一个表情，随机几个群友的头像各做一张，一条消息全发（`#全员` `#全体` 同效） |
| `#今日表情` | 同一个人同一天永远是同一个表情，配一句今日运势（`#今日运势` `#每日表情` 同效） |
| `#猜表情` | 出一张用你头像做的表情，限时说出名字得一分 |
| `#猜表情排行` | 本群猜对排行，带头像出图；`#猜表情总排行` 看跨群总榜 |
| `#meme搜索 猫` | 九宫格真图，多了会附一份可复制的名字列表 |
| `#meme列表` / `#meme分类 学习` | 随机翻一页（加页码看指定页）/ 按分类看，也可点机器人给的网页链接 |
| `#meme新增` | 最近更新的表情，带真图 |
| `#随机meme` | 随便来一个 |
